...
<table border="0" cellspacing="1" cellpadding="2">
    <tr>
      <td width="100" class="lightCell" align="center">Márka
	  <select name="m">
	  <option value="%">Összes</option>
	  <?
	  $m=mysql_query('SELECT id, markanev FROM markak ORDER BY markanev');
...
AND m.id like '".$_POST['m']."'
AND mf.id_statusz in (1, 2, 3)
...
WHERE mf.sztorno IS NULL 
AND mf.id_statusz <99
AND id_szallitasi_mod=1 
AND szallitasi_dij=0
AND DATE_FORMAT(mf.datum, '%Y-%m-%d') >= DATE_FORMAT('".date("Y-m-d", strtotime($datum_tol))."', '%Y-%m-%d')
AND DATE_FORMAT(mf.datum, '%Y-%m-%d') <= DATE_FORMAT('".date("Y-m-d", strtotime($datum_ig))."', '%Y-%m-%d')";
...
/* ARRES */
$arres='<table>
					<tr><td colspan=4 class="darkCell"><b>Árrés</b></td></tr>';
$arres.='<tr><td align="right" class="lightCell">Nettó eladási ár összesen: </td><td align="right" class="lightCell"><b>'.number_format($osszesen/1.27, 0, '', ' ').'</b> Ft</td></tr>';

$arres.='<tr><td align="right" class="lightCell">Nettó beszerzési ár összesen: </td><td align="right" class="lightCell"><b>-'.number_format($netto_besz_ar_ossz, 0, '', ' ').'</b> Ft</td></tr>';

$arres.='<tr><td align="right" class="lightCell">Nettó árrés összesen: </td><td align="right" class="lightCell"><b>'.number_format($netto_arres_ossz, 0, '', ' ').'</b> Ft</td></tr>';

$arres.='<tr><td align="right" class="lightCell">Árrés: </td><td align="right" class="darkCell"><b>'.number_format( ($netto_arres_ossz/($osszesen/1.27))*100, 0, '', ' ').'</b> %</td></tr>';

$arres.='<tr><td align="right" class="lightCell">Árrés az ingyenes szállítással csökkentve: </td><td align="right" class="darkCell"><b>'.number_format( ( ($netto_arres_ossz-($free_shipping_qty*850)) / ($osszesen/1.27))*100, 0, '', ' ').'</b> %</td></tr>';
...
if (is_array($forgalom)){
    ...
    while ($adatok = each($forgalom)){
        ...
        $diagram.= '<tr>
                      <td class="darkCell" align="center" rowspan="4">'.$adatok[1]['datum'].'</td>
                    ...
                    <tr>
                      <td class="darkCell" align="right">Kedvezmény BRUTTÓ</td>
                      <td class="mediumCell">
					...
					<tr>
                      <td class="darkCell" align="right">Kedvezmény NETTÓ</td>
                      <td class="mediumCell">
                        <div style="position:relative; width:'.(int)$szazalek_kedvezmeny2.'%; background-color:#13506C; height:21px; margin: 1px">
                            <div style="position:absolute; width:200px; padding:3px">'.number_format(($adatok[1]['kedvezmeny']/1.27), 0, '', ' ').' Ft</div>
                        </div>
                      </td>
                    </tr>
					...
					<tr>
                      ...
                      <td colspan=2 class="darkCell" align="right" style="background-color:violet;">KEDVEZMÉNNYEL CSÖKKENTETT NETTÓ ÁRRÉS</td>
                      <td class="mediumCell" style="background-color:violet; color:#000;">
                        <b>'.number_format( ( $netto_arres_ossz - ($adatok[1]['kedvezmeny']/1.27)), 0, '', ' ').' Ft</b>
					...
					<tr>
                      ...
                      <td colspan=2 class="darkCell" align="right" style="background-color:pink;">KEDVEZMÉNNYEL és INGYEN SZÁLL. CSÖKKENTETT NETTÓ ÁRRÉS</td>
                      <td class="mediumCell" style="background-color:pink; color:#000;">
                        <b>'.number_format( ( $netto_arres_ossz - ($adatok[1]['kedvezmeny']/1.27) -($free_shipping_qty*850)), 0, '', ' ').' Ft</b>
