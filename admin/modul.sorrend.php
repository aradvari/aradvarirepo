...
echo '<form methon="GET" action="" />
		...
		<input type="hidden" name="lap" value="sorrend" />
		
		<table width="400" border="0" cellspacing="1" cellpadding="6">
		
		<tr><td class="lightcell" style="font-weight:bold; color:red;">termék sorrend</td></tr>
 ...
 $q = mysql_query("SELECT * FROM kategoriak WHERE szulo=0 AND sztorno IS NULL");
              ...
              while ($adatok=mysql_fetch_array($q)){
                  ...
                  $alquery = mysql_query("SELECT * FROM kategoriak WHERE publikus=1 AND szulo=".$adatok[0]." AND sztorno IS NULL");
...
$sql="SELECT
		...
		ORDER BY opcio_sorrend ASC, id DESC";
 
//$query = mysql_query($sql);
$numoftops = mysql_num_rows($query);

echo 'találat: '.$numoftops;
