			...
			if ( strlen( $teljes_nev[0] ) >= 2 )	{
				...
				if (  strlen( $teljes_nev[1] ) >3 )		{
					...
					if(!empty($_POST['iranyitoszam']))	{
						...
						if(!empty($_POST['utca_kozterulet']))	{

							$utca_kozterulet=explode(" ", $_POST['utca_kozterulet']);

							// kozterulet kereses
							foreach($utca_kozterulet as $search)	{					
															
									$query = 'SELECT * FROM kozterulet WHERE megnevezes LIKE LOWER("'.$search.'")';
									...
									if($rows>0)	{
										...
										$_POST['utcanev']=str_replace($search, '', $_POST['utca_kozterulet']);	// kozterulet elnevezes kulonveve
										
										$_POST['kozterulet']=$res[0];
										
										$kozterulet_nev=$search;
									...
									if($found==0)	{
										//$error->addError("Adj meg közterület nevet (Pl. út, utca, tér)");
										$hiba[]='Nem adtál meg közterület nevet (Pl. "<b>'.ucfirst($_POST['utca_kozterulet']).'</b>" út, utca, tér)';
									}
								
							if(empty($_POST['hazszam']))	{
								//$error->addError("Házszám megadása kötelező!");	
								$hiba[]="Házszám megadása kötelező!";	
							}
...
onchange=\"divHelysegek('helysegekdiv', this.options[this.selectedIndex].value, 'varos')\" style=\"width:68.8%;v_isibility:hidden;\" ", "Válassz megyét");
?>
...
<input type="text" name="utca_kozterulet" id="utca_kozterulet" placeholder="Utcanév (Pl.: Kossuth utca)" value="<?=ucfirst($_POST['utca_kozterulet'])?>" style="width:78%;" onChange="document.getElementById('hazszam').focus();" style="text-transform:initial;" />

<input type="text" name="hazszam" id="hazszam" placeholder="Házszám" value="<?=$_POST['hazszam']?>" style="width:21%;float:right;" onChange="document.getElementById('email').focus();" />

<? /* <input type="text" name="emelet" id="emelet" placeholder="<?=$lang->Emelet_ajto_egyeb?>" value="<?=$_POST['emelet']?>" /> */ ?>

<p>Elérhetőség</p>
<input type="text" name="email" id="email" placeholder="<?=$lang->Email_cim?>" value="<?=$_POST['email']?>" onChange="document.getElementById('telefonszam1_1').focus();" />
