...
//    var $url = 'http://eki.cib.hu:8090/market.saki?';   //�LES
    var $url = 'http://ekit.cib.hu:8090/market.saki?';    //TESZT
//    var $url2 = 'https://eki.cib.hu/customer.saki?';    //�LES
    ...
    var $redir = "";
    var $userId;
    var $csh_file = '../extensions/cib/cib.security/cib_test/CSH.des';
//    var $csh_file = '../extensions/cib/cib.security/cib/CSH.des';    //�LES
    ...
    function wecho($str)
    {
        ...
        $f = fopen("../extensions/cib/log.txt", "a");
        fwrite($f, $str . "\n");
        fclose($f);
    ...
    function msg10($id, $trid, $uid, $amo, $timestamp)
    {
        ...
        $sql = "INSERT INTO bank_tranzakciok (id_megrendeles_fej, id_felhasznalo, datum, ip_cim, trid, uid, amo, cur, ts) VALUES ($id, " . $this->userId . ", NOW(), '" . \Yii::$app->request->getUserIP() . "', $trid, '$uid', '$amo', '$this->cur', '$timestamp')";

        if (!Yii::$app->db->createCommand($sql)->execute() && $this->debug) $this->wecho($sql . " - " . mysql_error());
        ...
        if ($this->debug) $this->wecho("�ssze�llatott URL:<br>" . $osszeallitott_url . "<hr>");

        $out = $this->ekiEncodeUrl($params, $this->csh_file);
        if ($this->debug) $this->wecho("K�dolt URL:<br>" . $this->url . $out . "<hr>");

        $lines = $this->get_web_page($this->url . $out, "");
        if ($this->debug) $this->wecho("V�lasz �zenet:<br>" . $lines . "<hr>");

        $out = $this->ekiDecodeUrl($lines, $this->csh_file);
        if ($this->debug) $this->wecho("Dek�dolt v�lasz:<br>" . $out . "<hr>");
        ...
        if ($parse['RC'] == "00") {
            ...
            if ($this->debug) $this->wecho("�ssze�ll�tott param�terek:<br>" . $pp . "<hr>");

            $outt = $this->ekiEncodeUrl($pp, $this->csh_file);

            if ($this->debug) $this->wecho('V�s�rl� URL:<br><a href="' . $this->url2 . $outt . '">' . $this->url2 . $outt . '</a>');
    ...
    function msg32($trid, $amo)
    {
        ...
        if ($this->debug) $this->wecho("�ssze�llatott param�terek:<br>" . $params . "<hr>");

        $out = $this->ekiEncodeUrl($params, $this->csh_file);
        if ($this->debug) $this->wecho("K�dolt param�terek:<br>" . $out . "<hr>");
    ...
    /*
            Titkos�t�s a saki protokoll alapj�n

            Param�terek:

                $plaintext: a titkos�tand� string
                $keyfile: a titkos�t�shoz sz�ks�ges kulcsfile neve
    ...
    /*
        Kititkos�t�s a saki protokoll alapj�n

        Param�terek:

        	$crypto: a kititkos�tand� string
        	$keyfile: a titkos�t�shoz sz�ks�ges kulcsfile neve

        Visszat�r� �rt�k:

            A kititkos�tott �zenet, vagy crc hiba eset�n �res string
